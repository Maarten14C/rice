...
Radiocarbon dating requires a range of calculations, for example calibration[^1][^2][^3][^4], translations between pMC, $\text{F}^{14}\text{C}$, $^{14}\text{C}$ age and $\Delta^{14}\text{C}$, and assessing the impacts of contamination. This package provides functions to do so in R.
...
The nuclei of atoms are made up of a combination of protons and neutrons - the number of neutrons then determines the isotope. We can check what happens when different amounts of protons and neutrons are joined into a nucleus:
...
C-14 or radiocarbon is a radioactive isotope that decays with a half-life of around 5730 years, and which is commonly used to date organic material back to around 50,000 years ago. 

As cosmic rays interact with N-14, they produce C-14 at a global rate of around 6 kg C-14 per year. As can be seen in the first panel of the below Figure, if C-14 were stable and allowed to fill the atmosphere (as 14CO2) at that rate for 50,000 years, it'd reach 300,000 kg. However, since C-14 decays at a constant rate, over time an equilibrium would be reached between production and decay (top-right panel). Additionally, C-14 does not only accumulate in the atmosphere, but also diffuses into the ocean which is much larger than the atmosphere (bottom-left panel). 14C is usually expressed as the 14C/12C ratio (bottom-right). The dashed line is the atmospheric 14C/12C ratio in AD 1950 (since the ocean has lower 14C/12C ratios, its 14C ages are older).

```{r C14.cycle, fig.width=7, fig.asp=.8, dpi=100}
layout(matrix(1:4, ncol=2, byrow=TRUE))
C14.cycle(as.ratio=FALSE, exchange=0, halflife=Inf) # top-left panel
C14.cycle(as.ratio=FALSE, exchange=0) # top-right
C14.cycle(as.ratio=FALSE) # bottom-left
C14.cycle() # bottom-right
```

Given a starting concentration $A_0$ of some organic material that we want to find the age of, the remaining amount of radiocarbon at time $t$ can be found as $A_t = A_0 e^{-\lambda t}$:

```{r howmuchC14, fig.width=4, fig.asp=.8}
howmuchC14(0)
howmuchC14(55e3)
x <- seq(0, 55e3, length=100) # a sequence of ages
y <- sapply(x, function(x) howmuchC14(x, talk=FALSE)[1])
plot(x, y, type="l", xlab="time (cal BP)", ylab="C-14 remaining")
```

Younger samples will have more C-14 atoms left than older ones, and thus will get higher counts-per-second at the AMS's C-14 detector. Want to hear a 'sonification' of the counts-per-second of C-14 atoms? The frequency of the tone relates to the counts per second, and the clicks represent the particles crashing into the detector. A visualisation of the counts as well as how they translate into a C-14 age is also provided. (This requires the 'audio' R package to be installed; if it isn't, a notification will be displayed with installation instructions.)
...
The radiocarbon dating of material of a certain calendar age (e.g., 9000 cal BP) can be simulated quickly and cheaply as:

```{r sim.date}
sim.date(9000, 10) # 10 dates
```
...
```{r fractionation_and_background}
...
```{r draw_calibration_curve_1, fig.width=4, fig.asp=.8}
draw.ccurve()
grid()
abline(0, 1, lty=2)
```
...
```{r timescales_2, fig.width=4, fig.asp=.8}
draw.ccurve(1600, 1950, BCAD=TRUE)
...
```{r draw_ccurve_1, fig.width=4, fig.asp=.8}
...
```{r draw_ccurve_2, fig.width=4, fig.asp=.8}
draw.ccurve(1600, 2020, BCAD=TRUE, cc2='nh1', add.yaxis=TRUE)
```

The calibration curves can also be plotted in the timescales of F14C, pMC or $\Delta{14}\text{C}$ (see below), e.g.:

```{r draw_ccurve_3, fig.width=4, fig.asp=.8}
draw.ccurve(50000, 35000, timescale="D")
```

Some older calibration curves are also available for historical interest, e.g.:

```{r draw_ccurve_4, fig.width=4, fig.asp=.8}
...
This package provides functions to translate values between the radiocarbon-relevant timescales of cal BP (calendar years before AD 1950), BC/AD (here we mean cal BC/AD, but shorten this to BC/AD), b2k (years before AD 2000, popular in the ice core community), C14, $\text{F} ^{14} \text{C}$, pMC and $\Delta ^{14} \text{C}$. The following Table lists the available functions:

| from\\to  | calBP         | BCAD          |  b2k         | C14          | F14C         | pMC         | Delta14C     |
|-----------|---------------|---------------|--------------|--------------|--------------|-------------|--------------|
| **calBP** | -             | `calBPtoBCAD` | `calBPtob2k` | `calBPtoC14` | `calBPtoF14C`| `calBPtopMC`| `calBPtoDelta14C`|
| **BCAD**  | `BCADtocalBP` | -             | `BCADtob2k`  | `BCADtoC14`  | `BCADtoF14C` | `BCADtopMC` | `BCADtoDelta14C` |
...
Delta14C ($\Delta$^14^C, a proxy for atmospheric ^14^C concentration at *t* cal BP) can be transferred to F^14^C, and the other way around:
...
```{r draw.CF, fig.width=5, fig.asp=1}
draw.CF(50000, 3000) 
```

---

Here's a function to transform values between multiple timescales and visualise them:

```{r fromto, fig.width=8, fig.asp=.6}
fromto(100, "calBP")
```
...
```{r draw.D14C, fig.width=4, fig.asp=.8}
...
We can also use draw.ccurve to plot the IntCal20 curve as both C-14 years and $\Delta^{14}\text{C}$:

```{r drawcurve, fig.width=6, fig.asp=.8}
...
```{r as.one, fig.width=5, fig.asp=.8}
...
```{r overlap, fig.width=5, fig.asp=.8}
y <- c(3820, 4430+90)
...
The calibrated ranges of radiocarbon dates can also be displayed in a table (either on a webbrowser or as Word document):

```{r calibratable, fig.width=4, fig.asp=1}
data(shroud)
calibratable(shroud$y, shroud$er, shroud$ID, BCAD=TRUE)
```

# Contamination

To calculate the effect of contamination on radiocarbon ages, e.g. what age would be observed if material with a "true" radiocarbon age of 5000 &pm; 20 ^14^C BP would be contaminated with 10\% of modern carbon (F^14^C=1)?

```{r contaminate_1, fig.width=5, fig.asp=.8}
...
```{r contaminate_2, fig.width=5, fig.asp=.8}
...
```{r clean_1, fig.width=5, fig.asp=.8}
clean(9000, 100, 10)
```
...
```{r muck_1, fig.width=5, fig.asp=.8}
muck(591, 30, BCADtoC14(40)[,1], 0, 1)
```

So we'd require the sample to have been contaminated by 67% modern carbon to still date to around AD 40. But what if the sample had been repaired in, say, the AD 1532 fire, thus adding material of an not-entirely-modern F14C value (i.e., taking into account both the atmospheric C-14 concentrations in AD 1532 and the fact that some of the C14 will have decayed since then)?

```{r muckshroud, fig.width=5, fig.asp=.8}
...
```{r draw.contamination_2, fig.width=6, fig.asp=.8}
draw.contamination()
```
...
```{r calib_1, fig.width=4, fig.asp=.8}
...
To sample n=100 random values from a calibrated distribution (where the probability of a year being sampled is proportional to its calibrated height):

```{r r.calib, fig.width=4, fig.asp=.8}
...
```{r calib_plot, fig.width=4, fig.asp=.8}
...
```{r calibrate_1, fig.width=5, fig.asp=1}
calibrate(2450, 40)
```
...
```{r calibrate_2, fig.width=5, fig.asp=1}
...
```{r calibrate_3, fig.width=5, fig.asp=1}
...
```{r push.normal, fig.width=5, fig.asp=1}
...
```{r map.shells_1, eval=FALSE}
myshells <- map.shells(S=54, W=-8, N=61, E=0) # the northern part of the UK
```

```{r map.shells_2, fig.width=5, fig.asp=1, echo=FALSE}
...
If you don't have the R package 'rnaturalearth' installed, you will be asked to install it if possible, because the resulting maps will look much nicer. Same for the github package 'rnaturalearthhires', which produces more detailed maps.

The output can also be queried:

```{r shells.mean_1, eval=FALSE}
head(myshells)
shells.mean(myshells)
```

```{r shells.mean_2, fig.width=5, fig.asp=1, echo=FALSE}
myshells <- map.shells(S=54, W=-8, N=61, E=0, currents=FALSE) # the northern part of the UK
head(myshells)
shells.mean(myshells)
```

You can also extract say the 20 shells closest to a coordinate, e.g., 20 East and 30 South:
```{r, eval=FALSE}
myshells <- find.shells(20, -30, 20)
shells.mean(myshells, distance=TRUE)
```

```{r shells.mean_3, fig.width=5, fig.asp=1, echo=FALSE}
...
```{r find.shells, fig.width=5, fig.asp=1, eval=FALSE}
...
The `p3k14c` R package[^9] contains over 175,000 radiocarbon dates (!) and is available on github[^10] (at c. 25 MB it's too big for CRAN). Within `rice`, the data from `p13k14c` can be downloaded, plotted and browsed as follows:

```{r p3k14c, fig.width=5, fig.asp=1, eval=FALSE}
p3k14c <- map.dates()
```
<img src="./p3k14c.png" width="600" />

For another "database of databases", have a look at c14bazAAR[^11] which is also on github (and too big for CRAN). After installing that package from github, type `c14bazAAR::db_info_table` to see a table of available databases.

# Multiple calibrations

It is also possible to draw multiple calibrated distributions:

```{r draw.dates_1, fig.width=4, fig.asp=1}
set.seed(123)
...
```{r draw.dates_2, fig.width=4, fig.asp=1}
...
or get creative (inspired by Jocelyn Bell Burnell[^12], Joy Division[^13] and the Hallstatt Plateau[^14]):

```{r joy, fig.width=5, fig.asp=1}
n <- 50; set.seed(42) # number of dates; always the same answer
from <- 2200; to <- 2900 # calendar age limits
y <- rnorm(n, 2620, 30) - 6*(1:n) # going through the Hallstatt Plateau

# set up an empty plot with black background
par(bg="black", mar=rep(1,4))
plot(0, type="n", xlim=c(to, from), ylim=c(n,-2), xlab="", ylab="")

for(i in 1:n) { # for each date...
  tmp <- draw.dates(y[i], 20, i, add=TRUE,
    mirror=FALSE, draw.hpd=FALSE, col="black", border=NA,
    peak=4, as.unit=TRUE) # distributions in black
  outline <- approx(tmp$ages, tmp$prob, seq(from, to, length=500), rule=2)
  y0 <- rnorm(500, i, 0.05) # make some noise
  lines(outline$x, y0-4*outline$y, col="white")
}
```


[^1]: Stuiver, R., Polach, H.A., 1977. Discussion: reporting of 14C data. *Radiocarbon* 19, 355-363 <http://dx.doi.org/10.1017/S0033822200003672>

[^2]: Reimer, P.J., Brown, T.A., Reimer, R.W., 2004. Discussion: reporting and calibration of post-bomb 14C Data. *Radiocarbon* 46, 1299-1304 <http://dx.doi.org/10.1017/S0033822200033154>
...
[^9]: Bird, D., Miranda, L., Vander Linden, M. et al., 2022. p3k14c, a synthetic global database of archaeological radiocarbon dates. *Scientific Data* 9, 27. <https://doi.org/10.1038/s41597-022-01118-7>

[^10]: https://github.com/people3k/p3k14c

[^11]: https://github.com/ropensci/c14bazAAR

[^12]: https://www.cam.ac.uk/stories/journeysofdiscovery-pulsars

[^13]: https://www.radiox.co.uk/artists/joy-division/unknown-pleasures-album-cover-meaning/

[^14]: https://en.wikipedia.org/wiki/Hallstatt_plateau
