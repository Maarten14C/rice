...
With multiple dates, we can assess the overlap between their calibrated distributions (as the minimum calibrated height for each calendar year in a range). For example, imagine two dates from the same archaeological layer, a twig (3820 +- 40, requiring calibration with IntCal20) and a marine shell (4430 +- 40, requiring Marine20, with a regional marine offset of 90 +- 25). To what degree do their calibrated distributions overlap?
...
draw.dates(rnorm(n, 2450, 30), rep(25, n), 1:n,
  ...
  threshold=1e-28, age.lim=c(2250, 2800), up=FALSE, ex=-12)
```
