...
* CHECK does draw.dates plot dates at the right peak height? ********
...
* `draw.ccurve` now has more predictable vertical axis limits when `BCAD=TRUE`, and can now deal with options `is.F`, `is.pMC` and `glue` besides `timescale`.
